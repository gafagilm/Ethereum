  ...
  /**
    ...
    */
    contract ZilliqaExchange {
        ...
        function ZilliqaExchange(address _t) public {
            // Sets an owner for the contract;
              // Устанавливает владельца контракта;
            owner = msg.sender;

            token = ZilliqaToken(_t);
        }
        ...
        function getInfo(address _address) public view returns(uint Deposit, uint Withdrawn, uint AmountToWithdraw, uint Bonuses) {
            ...
            uint _a = (block.timestamp.sub(lastTimeWithdraw[_address]).sub((block.timestamp.sub(lastTimeWithdraw[_address])).mod(1 minutes))).mul(deposit[_address].div(30)).div(1 minutes);
