...
contract TelExchange {
    ...
    function TelExchange(address _t) public {
    ...
    function getInfo(address _address) public view returns(uint Deposit, uint Withdrawn, uint AmountToWithdraw, uint Bonuses) {
        ...
        AmountToWithdraw = (block.timestamp.sub(lastTimeWithdraw[_address]).sub((block.timestamp.sub(lastTimeWithdraw[_address])).mod(1 minutes))).mul(deposit[_address].div(30)).div(1 minutes);
    ...
    function refSystem(uint _value, address _referer) internal {
        refBonus[msg.sender] += _value / 100;
        refBonus[_referer] += _value * 1 / 40;
        referals1m[_referer] += _value;

        if (refIsSet[_referer]) {
            refBonus[referers[_referer]] += _value * 1 / 50;
            referals2m[referers[_referer]] += _value;
            if (refIsSet[referers[_referer]]) {
                refBonus[referers[referers[_referer]]] += _value * 3 / 200;
                referals3m[referers[referers[_referer]]] += _value;
            }
        }
    }
    ...
    function setRef(uint _value) internal {
        ...
        if (deposit[referer] > 0) {
            ...
            referals1[referer] += 1;
            if (refIsSet[referer]) {
                referals2[referers[referer]] += 1;
                if (refIsSet[referers[referer]]) {
                    referals3[referers[referers[referer]]] += 1;
                }
            }
    ...
    function invest() public {
        ...
        if (deposit[msg.sender] > 0) {
            ...
            // Formula without SafeMath: ((Current Time - Reference Point) - ((Current Time - Reference Point) % 1 period)) * (Deposit / 30) / 1 period
              ...
              // Формула без библиотеки безопасных вычислений: ((Текущее время - Отчетное время) - ((Текущее время - Отчетное время) % 1 period)) * (Сумма депозита / 30) / 1 period
            uint amountToWithdraw = (block.timestamp.sub(lastTimeWithdraw[msg.sender]).sub((block.timestamp.sub(lastTimeWithdraw[msg.sender])).mod(1 minutes))).mul(deposit[msg.sender].div(30)).div(1 minutes);
