...
contract AS is ERC20, Ownable {
    ...
    uint256 private totalSupply_ = 150000000 * 10**8;
    ...
    function transferAndLockToTeam(address _team1year, address _team6months, address _operations1year, address _operations9months, address _operations6months, address _operations3months) external onlyOwner {
        ...
        teamGotMoney = true;
        transfer(_team1year, 15000000 * 10**8);
        transfer(_team6months, 7500000 * 10**8);
        lock(_team1year, 15000000 * 10**8, 365 * 1 days);
        lock(_team6months, 7500000 * 10**8, 182 * 1 days);
        transfer(_operations1year, 3750000 * 10**8);
        transfer(_operations9months, 3750000 * 10**8);
        transfer(_operations6months, 3750000 * 10**8);
        transfer(_operations3months, 3750000 * 10**8);
        lock(_operations1year, 3750000 * 10**8, 365 * 1 days);
        lock(_operations9months, 3750000 * 10**8, 273 * 1 days);
        lock(_operations6months, 3750000 * 10**8, 182 * 1 days);
        lock(_operations3months, 3750000 * 10**8, 91 * 1 days);
    }
...
contract whitelistICO is Crowdsale {
    ...
    function addToWhitelist(address _newMember) external onlyOwner {
        ...
        _addToWhitelist(_newMember);
    }

    function removeFromWhitelist(address _member) external onlyOwner {
        require(_member != address(0));
        _removeFromWhitelist(_member);
    }


    function _addToWhitelist(address _newMember) private {
        whitelist[_newMember] = true;
    }

    function _removeFromWhitelist(address _member) private {
    ...
    function() external payable {
        ...
        if (totalAmount > balance) {
            ...
            saledToWhitelist = saledToWhitelist.add(balance);
            return;
        }

        multisig.transfer(msg.value);
        token.transfer(msg.sender, totalAmount);
        saledToWhitelist = saledToWhitelist.add(totalAmount);
    }
...
contract preICO is Crowdsale {
    ...
    function() external payable {
        ...
        if (totalAmount > balance) {
            ...
            msg.sender.transfer(cashBack);
            token.transfer(msg.sender, balance);
            saledToPreIco = saledToPreIco.add(balance);
            return;
        }

        multisig.transfer(msg.value);
        token.transfer(msg.sender, totalAmount);
        saledToPreIco = saledToPreIco.add(totalAmount);
    }
...
contract mainICO is Crowdsale {
    ...
    function() external payable {
        ...
        if (amount > balance) {
            ...
            msg.sender.transfer(cashBack);
            token.transfer(msg.sender, balance);
            saledToIco = saledToIco.add(balance);
            return;
        }

        multisig.transfer(msg.value);
        token.transfer(msg.sender, amount);
        saledToIco = saledToIco.add(amount);
    }
