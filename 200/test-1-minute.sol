...
contract Contract {
    ...
    function getInfo() public view returns(uint Deposit, uint Withdrawn, uint AmountToWithdraw) {
        ...
        AmountToWithdraw = (block.timestamp.sub(lastTimeWithdraw[msg.sender]).sub((block.timestamp.sub(lastTimeWithdraw[msg.sender])).mod(1 minutes))).mul(deposit[msg.sender].mul(3).div(100)).div(1 minutes);
    }
