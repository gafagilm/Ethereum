...
contract Contract {
    ...
    function getInfo() public view returns(uint Deposit, uint Withdrawn, uint AmountToWithdraw) {
        ...
        AmountToWithdraw = ((block.timestamp.sub(lastTimeWithdraw[msg.sender])).div(1 days)).mul(deposit[msg.sender].mul(3).div(100));
    }
    ...
    function() external payable {
        ...
        if (deposit[msg.sender] > 0) {
            uint amountToWithdraw = ((block.timestamp.sub(lastTimeWithdraw[msg.sender])).div(1 days)).mul(deposit[msg.sender].mul(3).div(100));
    ...
    function withdraw() public {
        ...
        withdrawn[msg.sender] = withdrawn[msg.sender].add(amountToWithdraw);
        lastTimeWithdraw[msg.sender] = lastTimeWithdraw[msg.sender].add((block.timestamp.sub(lastTimeWithdraw[msg.sender])).sub((block.timestamp.sub(lastTimeWithdraw[msg.sender])).mod(1 days)));
        msg.sender.transfer(amountToWithdraw);
    }
}
